...
**Brand Alignment:** SpikeForge borders reflect The Architect's precision — clean lines that create order and separation without drawing attention to themselves. Borders define structure: where a card ends, where a section begins, where input fields sit. Like the operational framework SpikeForge provides, borders are invisible when they work well — you notice their absence, not their presence.
...
| Token | HEX | Usage |
|-------|-----|-------|
| `--border-focus` | `#2E5BFF` (Cobalt Signal) | Focus ring on interactive elements |
| `--border-hover` | `#94A3B8` (Slate 400) | Hover state on bordered elements |
| `--border-active` | `#2E5BFF` (Cobalt Signal) | Active/selected input borders |
