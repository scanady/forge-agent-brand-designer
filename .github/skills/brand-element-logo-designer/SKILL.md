...
- **Brand strategy** — brand name, purpose, vision, mission, values, positioning, personality, archetypes, voice, story, brand promise, key messages
...
- **Typography** — primary typeface family and weight
- **Logo guidelines** (if provided) — logo concept vision, design principles, symbolism, and any structural intent described

**Do not skim the provided context.** Study it completely. A logo must encode the brand's soul into a single mark, so you need to understand every dimension of the brand before designing anything. If critical information is missing (brand name, core values, color palette), ask the user before proceeding.
...
### Step 3: Craft Image Generation Prompts
...
{If logo guidelines were provided, include:}
BRAND LOGO GUIDELINES:
Logo concept vision: {from provided logo guidelines}
Design principles: {from provided logo guidelines}
Symbolism intent: {from provided logo guidelines}
...
```bash
cd .github/skills/brand-element-logo-designer/scripts
python generate.py --prompt "<the full prompt you crafted>" --output "<output-dir>/logo-concept-<NN>-<concept-slug>.png"
```
...
### Step 5: Present Concepts to User
...
All generated images go in the output directory specified by the user, or a reasonable default like `elements-logo-designs/` in the current working area.
